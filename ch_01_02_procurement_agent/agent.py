...
# In a production system, this would be an ORM or API client integrating
# with an ERP system like SAP Ariba, Workday, or Oracle Financials.
# ---------------------------------------------------------------------------
_DB = get_mock_db()
...
def check_catalog(item_id: str) -> Dict[str, Any]:
    ...
    """
    normalized_item = item_id.lower().strip().replace(" ", "_")
...
root_agent = Agent(
    ...
    description="Tier 1 Enterprise Procurement Assistant enforcing strict financial and compliance boundaries.",
    instruction=(
